    ...
    def _profile_instructions(self, profile: str, assistant_name: str) -> List[str]:
        ...
        if profile == "packet_analyst":
            return [
                ...
                "If the context is too limited for certainty, say what is missing and what packet, stream, or protocol view the user should inspect next.",
                "Prefer concrete Wireshark actions such as display filters, Follow Stream, conversation views, endpoint views, and surrounding-frame inspection.",
                "When you provide a display filter, write it on its own line so it is easy to spot and copy.",
